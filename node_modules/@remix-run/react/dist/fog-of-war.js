...
function getPartialManifest(manifest, router$1) {
  ...
  paths.forEach(path => {
    let matches = router.matchRoutes(router$1.routes, path, router$1.basename);
    if (matches) {
...
async function fetchAndApplyManifestPatches(paths, errorReloadPath, manifest, routeModules, future, isSpaMode, basename, patchRoutes, signal) {
  ...
  parentIds.forEach(parentId => patchRoutes(parentId || null, routes.createClientRoutes(patches, routeModules, null, future, isSpaMode, parentId)));
}
...
exports.fetchAndApplyManifestPatches = fetchAndApplyManifestPatches;
